...
type ChainCategory = "mainnet" | "alt-l1" | "op-stack" | "nitro" | "zk" | "alt-l2";
...
const chainCategories = new Set<ChainCategory>(["mainnet", "alt-l1", "op-stack", "nitro", "zk", "alt-l2"]);
...
function validateRegistryChains(registryChains: RegistryChain[]): void {
  for (const chain of registryChains) {
    if (!chainCategories.has(chain.category)) {
      fail(`${chain.slug}.category must be mainnet, alt-l1, op-stack, nitro, zk, or alt-l2.`);
    }
